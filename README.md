...
> 请阅读整个仓库，理解露台与卧室两种体验的核心约束；根据我当前使用的平台，替我选择独立 Skill 或远程 MCP 的最简单安装方式。部署前把称呼、门牌和触发句改成适合我们的语言，不要删掉隐私边界、失败行为、来源可追溯性，以及“体验名不是实际地点或睡眠事实”的规则。

其余事情交给你的 AI 判断即可。它可以根据当时的平台能力调整安装、部署与界面，不需要机械复刻原作者的技术路径。
